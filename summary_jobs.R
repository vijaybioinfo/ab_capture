...
optlist <- list(
  ...
  make_option(
    opt_str = c("-p", "--pipeline"), type = "character", default = "./",
...
for(aggr in aggregations){
  my_aggregations <- if(dir.exists(aggr)){
    ...
    paste0(my_aggregations, "/outs/aggregation.csv")
  }else{ aggr }
  for(aggr_i in my_aggregations){
    ...
    if(file.exists(paste0(aggregation_name, ".rds")) && !isTRUE(grepl("force", config_file$submit))){
    ...
    if(isTRUE(any(c(config_file$job$submit, defargs$submit))) || isTRUE(grepl("^f|force", config_file$job$submit))){
